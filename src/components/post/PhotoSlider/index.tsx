...
function PhotoSlider({photosURL}: Props) {
    ...
    function onClickOver(e:React.TouchEvent<HTMLDivElement> | React.MouseEvent<HTMLDivElement>) {
        const itemWidth = e.currentTarget.offsetWidth;
        const currentIdx = Math.floor(e.currentTarget.scrollLeft / e.currentTarget.offsetWidth);
    ...
    return(
        ...
        >
            {
                photosURL.map((photoURL) => <S.PhotoItem src={photoURL} />)
            }
        </S.PlacePhotos>
    )
}
