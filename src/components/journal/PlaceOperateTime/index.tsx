...
function PlaceOperateTime({ opening_hours }: Props) {
  ...
  useEffect(() => {
    if (data) {
      const todayOperateInfo =
        data[`${date.getDay() === 0 ? 6 : date.getDay() - 1}`][1]
      const yesterdayOperateInfo =
        data[`${date.getDay() <= 1 ? date.getDay() + 5 : date.getDay() - 2}`][1]

      const { flag: todayCalcResult, displayEndTime: todayCalcDisplayTime } =
        calculateEndTime({ operateTimeInfo: todayOperateInfo })
      ...
      if (todayCalcResult) {
        ...
        const {
          flag: yesterdayCalcResult,
          displayEndTime: yesterdayCalcDisplayTime,
        } = calculateEndTime({
